...
contract CertificateNFT is ERC721URIStorage, Ownable, ReentrancyGuard {
   ...
   constructor() ERC721("Uganda Academic Certificate", "UGACERT") Ownable(msg.sender) {}
    ...
    function addAuthorizedInstitution(address institution, string memory name, string memory email) external onlyOwner {
        require(institution != address(0), "Invalid institution address");
        require(bytes(name).length > 0, "Name required");
        require(!institutions[institution].isAuthorized, "Already authorized");
    ...
    // 🔧 Fix: Mark _burn as overrideable and correctly override it
